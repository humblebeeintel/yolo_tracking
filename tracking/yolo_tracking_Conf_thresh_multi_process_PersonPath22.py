...
if __name__ == '__main__':
    ...
    trackers = ['ocsort', 'deepocsort', 'bytetrack', 'botsort']

    for conf_thresh in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7]:
        
        for tracker in trackers:
            args.tracking_method = tracker
            args.project = os.path.join('/media/hbai/data/code/LiteSORT/results/conf_thresh_experiment/PersonPath22-test', f"{tracker}__input_1280__conf_{conf_thresh}")
            
            print(f"Processing with Conf threshold {conf_thresh} using tracker {tracker}...")
