...
def process_sequence(seq_path, args, gpu_id, tracking_method):
    ...
    try:
        print(f'Processing sequence {seq_path} on GPU {gpu_id} as {device} with {tracking_method} (process ID: {os.getpid()})...', flush=True)
        ...
        args.project = os.path.join('/media/hbai/data/code/LiteSORT/yolo_tracking/hbai_scripts/VIRAT-S_LITE', f"{tracking_method}__input_1280__conf_.25")
        ...
        end_time = time.time()
        print(f'Finished processing sequence {seq_path} on GPU {gpu_id} as {device} with {tracking_method} in {end_time - start_time:.2f} seconds', flush=True)
    except torch.cuda.OutOfMemoryError as e:
        print(f'Error processing sequence {seq_path} on GPU {gpu_id} as {device} with {tracking_method} (process ID: {os.getpid()}): CUDA out of memory. {str(e)}', flush=True)
    except Exception as e:
        print(f'Error processing sequence {seq_path} on GPU {gpu_id} as {device} with {tracking_method} (process ID: {os.getpid()}): {str(e)}', flush=True)
...
if __name__ == '__main__':
    ...
    source_path = '/media/hbai/data/code/LiteSORT/datasets/VIRAT-S/train'
    tracking_methods = ['deepocsort', 'botsort']  # List of tracking methods
    ...
    for tracking_method in tracking_methods:
        print(f'Starting processing for tracking method: {tracking_method}', flush=True)
        ...
        print(f'Total time taken for {tracking_method}: {total_time:.2f} seconds', flush=True)

    overall_end_time = time.time()
    overall_total_time = overall_end_time - start_time
    print(f'Total time taken for all tracking methods: {overall_total_time:.2f} seconds', flush=True)
